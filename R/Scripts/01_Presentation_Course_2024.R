...
names(ls) <- c("Names", "Matrix", "Nested_List")
ls
...
# apply function (array or matrix)
m1 <- matrix(1:10, nrow = 5, ncol = 6)
m1

# For a matrix MARGIN = 1 indicates rows
# For a matrix MARGIN = 2 indicates columns

(a_m1 <- apply(m1, MARGIN = 2, sum))

apply(m1, MARGIN = 1, sum)

# lapply function (list or vector)
lapply(0:4, function(x) {x + 1})

# sapply function (is a user-friendly version and wrapper of lapply)
sapply(0:4, function(a) {a + 1})

# tapply function (ragged array)
...
pairs(penguin[ ,2:ncol(penguin)],
      ...
      lower.panel = panel.cor, #panel.cor(penguin[ ,2], penguin[ ,3])
